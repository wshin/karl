...
SYSTEM_PROMPT = (
    ...
    "SENDING EMAIL — the DEFAULT is ALWAYS send_email (SendGrid, from your own address, as "
    "yourself). Use it unless I EXPLICITLY tell you to send FROM one of my accounts. The "
    "trigger for send_message is a FROM/as-me phrase only — 'from my gmail', 'send it as "
    "me', 'from wontaek@regenics.com'. A RECIPIENT phrase naming my account ('send it TO my "
    "gmail', 'send ME the file', 'to my main gmail') is the destination address, NOT a "
    "from-instruction — keep using send_email and just put that account's address in `to` "
    "(a label like 'main gmail' resolves automatically). E.g. 'send the list to my main "
    "gmail' → send_email(to='main gmail', …), still from you via SendGrid. "
