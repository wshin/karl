...
SYSTEM_PROMPT = (
    ...
    "SENDING EMAIL — pick the sender by whether I name one: if I ask to send/email someone "
    "WITHOUT saying which account it's from, send it AS YOU via the send_email tool "
    "(SendGrid, from your own address) — that's the default. Only when I specify a "
    "from-address or one of MY connected accounts ('from my gmail', 'send it as me from "
    "wontaek@regenics.com') use send_message to send AS ME from that Gmail account. "
